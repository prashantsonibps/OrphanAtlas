...
fetched_data = {}
data_loaded = False   # <<< FIX: prevents loading huge Excel on startup
...
def index():
    ...
    global fetched_data, data_loaded
    if not data_loaded:
        fetch_diseases()
        data_loaded = True   # <<< FIX: load once, AFTER startup
...
def fetch_data():
    ...
    if sheet_name == "AI Generated Data":
        prompt = f"Generate 10 concise lines of hypothetical data about the disease '{query.capitalize()}' including prevalence, symptoms, treatments."
        try:
        ...
        if "Disease" in data.columns:
            ...
            if sheet_name == "Inheritance":
                results.append({"NOTE": "Not applicable = not inherited. Unknown = mode of inheritance not determined."})
            if sheet_name == "Prevalence":
                results.append({"NOTE": "Worldwide unless specified. * = European data."})
...
def fetch_data_stream():
    ...
    prompt = f"Generate 10 concise lines of hypothetical data about '{query.capitalize()}' including prevalence, symptoms, treatments."
    ...
    for sheet_name in sheets_to_include:
        ...
        if sheet_name == "Classification":
                        ...
                        for entry in unnamed_entries:
                            ...
                            if urls:
                                if len(urls) == 1:
                                    ...
                                    pdf.set_text_color(0, 0, 255)
                                    for i, url in enumerate(urls, 1):
                                        pdf.cell(0, 4, f"Click Link{i}", ln=(i == len(urls)), link=url)
                                    pdf.set_text_color(0, 0, 0)
                            else:
                                entry = truncate_text(entry, max_length=100)
            ...
            if sheet_name == "Prevalence":
                ...
                pdf.multi_cell(0, 4, "Note: Worldwide unless specified. * = European data.")
                pdf.ln(2)
            if sheet_name == "Inheritance":
                pdf.set_font("Arial", "I", 7)
                pdf.multi_cell(0, 4, "Note: Not applicable = not inherited.")
                pdf.ln(2)
...
def ask_bot():
    ...
    if user_query.lower().strip() in ["i'm really chatting with a person", "im really chatting with a person", "am i chatting with a person"]:
        response_text = """
I'm glad you feel that way! I'm OrphanAtlas Assistant, an AI designed to chat with you. While I'm not a human, I'm here to assist you 😊<br><br>
...
"""
    ...
    prompt = f"""
You are OrphanAtlas Assistant, developed by Prashant Soni.
...
"""
    ...
    try:
        response = deepseek_client.chat.completions.create(
            model="deepseek/deepseek-chat:free",
            messages=[
                {"role": "system", "content": "You reply with clean HTML for web."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=500,
            temperature=0.7
        ...
        if response and response.choices and response.choices[0].message:
            ...
            return jsonify({"answer": "Sorry, I couldn't generate a response."})
...
def get_disease_count():
    ...
    return jsonify({"count": 0, "error": "Prevalence sheet not found"}), 404

@app.route("/get_geographic_spread", methods=["POST"])
def get_geographic_spread():
    try:
        query = request.json.get("query", "").strip()
        prompt = f"List 10 countries where '{query}' is most common. Only respond with names, comma-separated."

        response = deepseek_client.chat.completions.create(
            model="deepseek/deepseek-chat:free",
            messages=[
                {"role": "system", "content": "List only country names."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=100,
            temperature=0.5
